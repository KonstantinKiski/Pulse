...
#define FINAL_RESULT_MARGIN 4.0
...
- (BOOL)shouldShowLatestResult
{
    if (self.isCalibrationOver && (self.framesCounter > self.calibrationDuration + self.firstPeakPlace + self.windowSize + self.windowSizeForAverageCalculation)) {
        ...
        double param1 = fabs([self.bpmAverageValues[self.framesCounter-self.calibrationDuration-self.windowSize - 1] doubleValue] - [self.bpmAverageValues[self.framesCounter-self.calibrationDuration-self.windowSize - 2] doubleValue]);
        NSNumber *number1 = [NSNumber numberWithDouble:param1];

        double param2 = fabs([self.bpmAverageValues[self.framesCounter-self.calibrationDuration/2-self.windowSize - 1] doubleValue] - [self.bpmAverageValues[self.framesCounter-self.calibrationDuration/2-self.windowSize - 2] doubleValue]);
        NSNumber *number2 = [NSNumber numberWithDouble:param2];
        
        double param3 = fabs([self.bpmAverageValues[self.framesCounter-self.windowSize - 1] doubleValue] - [self.bpmAverageValues[self.framesCounter-self.windowSize - 2] doubleValue]);
        NSNumber *number3 = [NSNumber numberWithDouble:param3];
        
        if (fabs([self.bpmAverageValues[self.framesCounter-self.calibrationDuration-self.windowSize - 1] doubleValue] - [self.bpmAverageValues[self.framesCounter-self.calibrationDuration-self.windowSize - 2] doubleValue]) < 0.15) { // def value 0.083
            
            if (fabs([self.bpmAverageValues[self.framesCounter-self.calibrationDuration/2-self.windowSize - 1] doubleValue] - [self.bpmAverageValues[self.framesCounter-self.calibrationDuration/2-self.windowSize - 2] doubleValue]) < 0.15) { // def value 0.066
                ...
                NSLog(@"%s", "NOOOOOOOOOO PARAMS 2sd");
                NSLog(@"%@",number1);
                NSLog(@"%@",number2);
                NSLog(@"%@",number3);
            }
        } else {
            NSLog(@"%s", "NOOOOOOOOOO PARAMS 1st");
            NSLog(@"%@",number1);
            NSLog(@"%@",number2);
            NSLog(@"%@",number3);
        }
